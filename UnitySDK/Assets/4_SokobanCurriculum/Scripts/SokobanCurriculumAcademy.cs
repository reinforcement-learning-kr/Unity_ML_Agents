...
public class SokobanCurriculumAcademy : Academy
{
    [HideInInspector]
    ...
    public bool IsGoalsBoxesOnSameEdge()
    {
        ...
        foreach (GameObject obj in actorObjs)
        {
            if (obj.CompareTag("box"))
            {
                ...
                {
                    bool isExist = false;
                    foreach (GameObject goal in actorObjs)
                    {
                ...
                else if (obj.transform.position.x == gridSize - 1)
                {
                    bool isExist = false;
                    foreach (GameObject goal in actorObjs)
                    {
                        if (goal.CompareTag("goal"))
                        {
                            if (goal.transform.position.x == gridSize - 1)
                            {
                                isExist = true;
                                break;
                            }
                ...
                else if (obj.transform.position.z == gridSize - 1)
                {
                    bool isExist = false;
                    foreach (GameObject goal in actorObjs)
                    {
                        if (goal.CompareTag("goal"))
                        {
                            if (goal.transform.position.z == gridSize - 1)
                            {
                                isExist = true;
                                break;
                            }
