...
public class Curriculum_SokobanAgent : Agent
{
    [Header("Specific to Sokoban")]
    private Curriculum_SokobanAcademy academy;
    ...
    {
        academy = FindObjectOfType(typeof(Curriculum_SokobanAcademy)) as Curriculum_SokobanAcademy;
    }
    ...
    {
        bool edge = academy.IsGoalsBoxesOnSameEdge();
        if (!edge)
        {
            Done();
            AddReward(-1f);
        ...
        if (blockTest.Where(col => col.gameObject.CompareTag("wall")).ToArray().Length == 0)
        {
            ...
            else if (blockTest.Where(col => col.gameObject.CompareTag("box")).ToArray().Length == 1)
            {
                ...
                else if (boxBlockTest.Where(col => col.gameObject.CompareTag("goal")).ToArray().Length == 1)
                {
                    ...
                    if (academy.RemoveBoxGoal(box, goal) == 0)
                    {
                        SetReward(2f);
                        Done();
                    }
